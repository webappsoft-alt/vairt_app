...
@Component({
  selector: 'app-invest-employment-info',
  templateUrl: './invest-employment-info.page.html',
  styleUrls: ['./invest-employment-info.page.scss'],
})
export class InvestEmploymentInfoPage implements OnInit {
  ...
  ngOnInit(){
    ...
    this.service.user_invest5(this.user_id,prop_id,invoice_id).subscribe(res=>{
      console.log(res)
    })
  }
  ...
  ionViewDidEnter(){
    ...
    this.service.get_employment_info(this.user_id).subscribe(res=>{
      console.log(res)
      this.status = res['employment']['employment_status']
      if(this.status=='self_employed'){
        this.self_employed=true;this.retired=false;this.student=false;this.not_employed=false
      } if(this.status=='retired'){
        this.self_employed=false;this.retired=true;this.student=false;this.not_employed=false
      } if(this.status=='student'){
        this.self_employed=false;this.retired=false;this.student=true;this.not_employed=false
      } if(this.status=='not_employed'){
        this.self_employed=false;this.retired=false;this.student=false;this.not_employed=true
      }
      this.recent_employer = res['employment']['recent_employment']
      this.position_held = res['employment']['recent_position']
      this.business_type = res['employment']['industry_type']
      this.source_income = res['employment']['soi']
     
      this.school = res['employment']['school']
      this.school_source_income = res['employment']['ssoi']
     
      this.business_name = res['employment']['business_name']
      this.title = res['employment']['your_title']
      this.business_type = res['employment']['business_type']
    })
  ...
  next(){
    if(this.status==undefined || this.status == '' || this.status == 'undefined'){
      this.showmessage('Please select employment status')
    }else{
    if(this.self_employed==true){
    if(this.business_name == undefined || this.business_name == '' || this.business_name == 'undefined'){
      this.showmessage('Please enter business name')
    }else if(this.title == undefined || this.title == 'undefined' || this.title == ''){
      this.showmessage('Please enter title')
    }else if(this.business_type == undefined || this.business_type == 'undefined' || this.business_type == ''){
      this.showmessage('Please enter business type') 
    }else{
      console.log(this.status)
    this.service.employment_info_self(this.user_id,this.status,this.business_name,this.title,this.business_type,this.status).subscribe(res=>{
      console.log(res)
    })
    if(this.work_address==false){
      this.service.update_user_address(this.autocomplete.input,this.country_code,this.city,this.state,
        this.postalCode,this.user_id).subscribe(res=>{
          console.log(res)
        })
    }
    if(this.service.getInvestdata()[0].invoice_id!=undefined){
      this.service.update_invest(this.user_id,this.service.getInvestdata()[0].property_id,this.service.getInvestdata()[0].invoice_id,'6').subscribe(res=>{
        console.log(res)
      })
    }else{
    let prop_id = window.localStorage.getItem('prop_id')
   ...
   this.service.setBackPage('invest-contact-info')
   this.appcomponent.leftSlide('invest-contact-info')
  }
  }
  if(this.student==true){
    if(this.school==undefined || this.school == '' || this.school == 'undefined'){
      this.showmessage('Please enter school') 
    }else if(this.source_income == undefined || this.source_income == '' || this.source_income == 'undefined'){
      this.showmessage('Please enter source of income') 
    }else{
   ...
   if(this.retired==true){
    console.log('retured')
    if(this.recent_employer==undefined || this.recent_employer=='undefined' || this.recent_employer == ''){
      this.showmessage('Please enter recent employment')
    }else if(this.position_held == undefined || this.position_held == 'undefined' || this.position_held == ''){
      this.showmessage('Please enter position held')
    }else if(this.business_type == undefined || this.business_type=='undefined' || this.business_type == ''){
      this.showmessage('Please enter business type')
    }else if(this.source_income == undefined || this.source_income == 'undefined' || this.source_income == ''){
      this.showmessage('Please enter source of income')
    }else{
  ...
  async showmessage(message){
    ...
    async getAddressFromCoords(lattitude, longitude) {
      ...
      this.nativeGeocoder.reverseGeocode(lattitude, longitude, options)
        .then((result: NativeGeocoderResult[]) => {
          ...
          let chk = this.countries1.filter(policy => policy.name === this.country);
          if(chk.length>0){
    ...
    setting(){
      this.service.setSettingBackPage('invest-employment-info')
