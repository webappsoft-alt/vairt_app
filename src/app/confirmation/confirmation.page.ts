...
export class ConfirmationPage implements OnInit {
  ...
  ngOnInit() {
    console.log(window.localStorage.getItem('share_id'))
    var today = new Date();
    ...
    this.payment_method  = window.localStorage.getItem('payment_method')
    ...
    this.share_amount = amount.toFixed(0)
    this.payment_status = 'Pending'
    this.prop_address = this.service.getPropertyDetail().address

    if(this.prop_address == undefined){
      console.log(this.service.getInvestdata())
      this.prop_address = this.service.getInvestdata()[0].address
    }
    this.service.get_user_data(this.user_id).subscribe(res=>{
      ...
      this.address = res['user'].user_address
      this.city = res['user'].user_city
      this.zip = res['user'].zip
      this.ssn = res['user'].ssn
  ...
  purchase(){
    ...
    console.log(window.localStorage.getItem('share_id'))
    if(this.invest_type=='flip'){
      let share_id = window.localStorage.getItem('share_id')
      this.service.update_sell_share(share_id).subscribe(res=>{
        console.log(res)
      })
    }
    if(this.service.getInvestdata()[0].invoice_id!=undefined){
      this.service.purchase_success(this.user_id).subscribe(res=>{
        console.log(res)
      })
      this.add_transaction()
      this.service.update_invest(this.user_id,this.service.getInvestdata()[0].property_id,this.service.getInvestdata()[0].invoice_id,'10').subscribe(res=>{
        console.log(res)
        this.loading = false
        this.showmessage('Congratulations!! you have successfully invested in property')
        this.add_transaction()
    ...
    this.service.user_invest10(this.user_id,prop_id,invoice_id).subscribe(res=>{
      console.log(res)
      this.loading = false
       this.showmessage('Congratulations!! you have successfully invested in property')
       this.add_transaction()
  ...
  add_transaction(){
    ...
    let curr_date = year+'/'+this.month+'/'+this.date

    let time = new Date()
    this.hours = time.getHours();
    if(this.hours<10){ this.hours = '0'+ time.getHours()}
     this.minutes = time.getMinutes();
    if(this.minutes<10){ this.minutes = '0'+ time.getMinutes()}
     this.seconds = time.getSeconds()
    if(this.seconds<10){ this.seconds = '0'+ time.getSeconds()}
     this.curr_time = this.hours+':'+this.minutes+':'+this.seconds
    let invest_amount = parseFloat(window.localStorage.getItem('ivestment_amount'))

    if(this.service.getPropertyDetail().net_dividend == undefined){
      this.net_dividend = this.service.getInvestdata()[0].net_dividend
    }else{
      this.net_dividend = parseFloat(this.service.getPropertyDetail().net_dividend)
    }
    let prop_id = window.localStorage.getItem('prop_id')
    let invoice_id = window.localStorage.getItem('invoice_id')
    this.service.add_user_transaction(this.user_id,invest_amount,curr_date,this.curr_time,this.net_dividend,prop_id,invoice_id).subscribe(res=>{
      console.log(res)
    })
  }
