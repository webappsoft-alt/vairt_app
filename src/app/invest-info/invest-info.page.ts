...
@Component({
  ...
  templateUrl: './invest-info.page.html',
  styleUrls: ['./invest-info.page.scss'],
})
export class InvestInfoPage implements OnInit {
  ...
  insert_data(){
    ...
    let check_from = this.service.get_invest_form()
    let seller_id;
    if(check_from=='flip'){
        seller_id = window.localStorage.getItem('share_id')
    }else{
      seller_id = '0'
    }
    this.service.user_invest(this.user_id,prop_id,this.first_name,this.last_name,
      this.phone_no,this.user_address,this.zip,seller_id).subscribe(res=>{
      window.localStorage.setItem('invoice_id',res['invoice_id'])
      console.log(res['invoice_id'])
    })
  }
  next(){
    if(this.country_code==undefined || this.country_code=='undefined'|| this.country_code==0){
      this.toast_('Country Residence is required'); 
    }else if(this.user_address==undefined || this.user_address=='undefined' || this.user_address==''){
      this.toast_('Address is required');
    }else if(this.city==undefined || this.city=='undefined' || this.city==''){
      this.toast_('City is required');
    }else if(this.zip == undefined || this.zip=='undefined' || this.zip==undefined){
      this.toast_('Zip is required')
    }else{
    if(this.service.getInvestdata()[0].invoice_id!=undefined){
      this.service.update_invest(this.user_id,this.service.getInvestdata()[0].property_id,this.service.getInvestdata()[0].invoice_id,'1').subscribe(res=>{
        console.log(res)
      })
    }
    ...
    this.service.setBackPage('invest-info')
  ...
  async showmessage(message){
    ...
    async getAddressFromCoords(lattitude, longitude) {
      ...
      this.nativeGeocoder.reverseGeocode(lattitude, longitude, options)
        .then((result: NativeGeocoderResult[]) => {
          ...
          let chk = this.countries.filter(policy => policy.name === this.user_country);
          if(chk.length>0){
    ...
    back(){
      ...
      this.appcomponent.downSlidePre(page)
        ...
        this.appcomponent.downSlidePre('property-detail')
      }
    }
    home(){
