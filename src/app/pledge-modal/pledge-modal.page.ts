...
@Component({
  ...
  templateUrl: './pledge-modal.page.html',
  styleUrls: ['./pledge-modal.page.scss'],
})
export class PledgeModalPage implements OnInit {
...
prop_id:any
  ...
  next(){
    ...
    let pledge_amount = this.navprams.data.pledge_amount
    this.title = this.navprams.data.title
    this.prop_id = this.navprams.data.prop_id
    this.user_id  = window.localStorage.getItem('user_id')
    console.log(pledge_amount)
    this.service.pledge_email(this.first_name,this.last_name,this.email,this.phone_no,this.address,pledge_amount,this.user_country,this.city,this.state,this.zip,this.user_id,
      this.title,this.prop_id).subscribe(res=>{
      console.log(res)
    })
