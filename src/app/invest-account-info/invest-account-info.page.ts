...
declare var google;
@Component({
  selector: 'app-invest-account-info',
  templateUrl: './invest-account-info.page.html',
  styleUrls: ['./invest-account-info.page.scss'],
})
export class InvestAccountInfoPage implements OnInit {
  acc_type:any
  img:any
  purchase_price:any
  remaining_share:any
  ...
  joint_contact_same(){
    if(this.joint_inv_address==true){
      this.joint_phone_no = this.phone_no
      this.country = this.country_code
      let chk = this.countries.filter(policy => policy.code === this.country);
      if(chk.length>0){this.joint_country = chk[0].name;}
      this.autocomplete.input= this.user_address
      this.city = this.city 
      this.postalCode = this.zip
    }else{
      this.joint_first_name = ''
      this.joint_last_name = ''
      this.joint_phone_no = ''
      this.country = ''
      this.autocomplete.input= ''
      this.city = ''
  ...
  ngOnInit() {
    this.user_id = window.localStorage.getItem('user_id')
     this.prop_id = window.localStorage.getItem('prop_id')
      let invoice_id = window.localStorage.getItem('invoice_id')
      if(invoice_id==undefined){
        invoice_id = this.service.getInvestdata()[0].invoice_id
      }
      this.service.user_invest2(this.user_id,this.prop_id,invoice_id).subscribe(res=>{
        console.log(res)
      })
    ...
    this.service.get_user_data(this.user_id).subscribe(res=>{
      ...
      if(this.acc_type=='individual'){
        this.individual=true;
        this.joint=false
        this.trust=false;
        this.entity=false;
        this.retirement=false;
      } if(this.acc_type=='joint'){
        this.individual=false;
        this.joint=true
        this.trust=false;
        this.entity=false;
        this.retirement=false;
        this.service.get_joint_type(this.user_id,this.prop_id).subscribe(res=>{
          console.log(res)
          this.joint_first_name = res['values']['first_name']
          this.joint_last_name = res['values']['last_name']
          this.joint_inv_address = res['values']['contact_info']
          this.joint_owner = res['values']['pid']
          this.joint_phone_no = res['values']['phone']
          let chk = this.countries.filter(policy => policy.code === res['values']['c_residence']);
          if(chk.length>0){this.country = chk[0].name;}
          else{ this.country = ''}
          this.state = res['values']['state']
          this.autocomplete.input = res['values']['address']
          this.postalCode = res['values']['zip']
          this.primary_owner = res['values']['pid']
        })
      } if(this.acc_type=='trust'){
        this.individual=false;
        this.joint=false
        this.trust=true;
        this.entity=false;
        this.retirement=false;
        this.service.get_trust_type(this.user_id,this.prop_id).subscribe(res=>{
          console.log(res)
          this.name_trust = res['values']['name']
          this.title_trust = res['values']['title']
          this.trust_type = res['values']['type']
          this.tin = res['values']['tin']
          this.tin_select =  res['values']['jurisdiction']
          this.upload_file = res['values']['doc_title']
          if(res['values']['doc_later']==0){this.doc_later = false}
          else{this.doc_later=true}
        })
      } if(this.acc_type=='entity'){
        this.individual=false;
        this.joint=false
        this.trust=false;
        this.entity=true;
        ...
        this.service.get_trust_type(this.user_id,this.prop_id).subscribe(res=>{
          console.log(res)
          this.name_entity = res['values']['name']
          this.title_entity = res['values']['title']
          this.entity_type = res['values']['type']
          this.certification_file = res['values']['doc_title']
          this.operating_file = res['values']['doc_title1']
          if(res['values']['doc_later']==0){this.doc_entity = false}
          else{this.doc_entity=true}
        })
      } if(this.acc_type=='retirement'){
        this.individual=false;
        this.joint=false
        this.trust=false;
        this.entity=false;
        this.retirement=true;
        this.service.get_retirement_type(this.user_id,this.prop_id).subscribe(res=>{
          console.log(res)
          this.retirement_value = res['values']
        })
      }
      let chk = this.countries.filter(policy => policy.code === res['user'].user_country);
      if(chk.length>0){
  ...
  next(){
    this.service.setBackPage('invest-account-info')
    if(this.acc_type==undefined || this.acc_type==''){
      this.toast_('Please select Account Type')
    }else{
      this.service.update_account_type(this.acc_type,this.user_id).subscribe(res=>{
        console.log(res)
      })
      if(this.acc_type=='individual'){
        this.update_steps()
      ...
      if(this.acc_type=='joint'){
         if(this.joint_phone_no==undefined || this.joint_phone_no=='undefined' || this.joint_phone_no == ''){
          this.toast_('Phone No is required')
        }else if(this.country_code==0 || this.country_code == undefined  || this.country_code=='undefined' || this.country_code == ''){
          this.toast_('Country Residence')

        }else if(this.autocomplete.input==undefined || this.autocomplete.input=='undefined' || this.autocomplete.input == ''){
          this.toast_('Address is required')
        }else if(this.state==undefined || this.state=='undefined' || this.state == ''){
          this.toast_('State is required')
        }else if(this.postalCode==undefined || this.postalCode=='undefined' || this.postalCode == ''){
          this.toast_('Zip Code is required')
        }else{
          this.joint_()
          this.update_steps()
          this.appcomponent.leftSlide('invest-amount')
      ...
      if(this.acc_type=='trust'){
        if(this.name_trust==undefined || this.name_trust=='undefined' || this.name_trust == ''){
          this.toast_('Trust name is required')
        }else if(this.title_trust==undefined || this.title_trust=='undefined' || this.title_trust == ''){
          this.toast_('Title Trust is required')
        }else if(this.trust_type==undefined || this.trust_type=='undefined'  || this.trust_type == ''){
          this.toast_('Trust Type is required')
        }else if( this.tin == undefined  || this.tin=='undefined' || this.tin == ''){
          this.toast_('Tin is required')
        }else if(this.tin_select==undefined || this.tin_select=='undefined' || this.tin_select == ''){
          this.toast_('Tin Type is required')
        }else{
      ...
      if(this.acc_type=='entity'){
        if(this.name_entity==undefined ||  this.name_entity=='undefined'  || this.name_entity == ''){
          this.toast_('Entity name is required')
        }else if(this.title_entity==undefined || this.title_entity=='undefined' || this.title_entity == ''){
          this.toast_('Entity title is required')
        }else if(this.entity_type==undefined || this.entity_type=='undefined' || this.entity_type == ''){
          this.toast_('Entity type is required')
        }else{
      ...
      if(this.acc_type=='retirement'){
        if(this.retirement_value==undefined || this.retirement_value=='undefined' || this.retirement_value == ''){
          this.toast_('Retirement value is required')
        }else{
          this.retirement_()
          this.update_steps()
          this.appcomponent.leftSlide('invest-amount')
  ...
  update_steps(){
    ...
    if(this.service.getInvestdata()[0].invoice_id!=undefined){
      this.service.update_invest(this.user_id,this.service.getInvestdata()[0].property_id,this.service.getInvestdata()[0].invoice_id,'3').subscribe(res=>{
        console.log(res)
      })
    }else{
     let invoice_id = window.localStorage.getItem('invoice_id')
     console.log(invoice_id)
     this.service.user_invest2(this.user_id,prop_id,invoice_id).subscribe(res=>{
      console.log(res)
    })
   }
  }
    ...
    //   return await modal.present(); 
     ...
     async top_down(){
      ...
      // this.firstname.setFocus();
        this.firstname_edit = false;
        this.lastname_edit = false;
        this.country_edit = false;
        this.city_edit = false;
        this.address_edit = false;
        this.phoneno_edit=false
        this.edit_btn = false;
        this.zipcode_edit=false
      }
  update(){
    this.edit_btn = true;
  ...
  async showmessage(message){
    ...
    trust_file_upload(){
      ...
      this.service.update_type_trust(this.user_id,this.name_trust,this.title_trust,this.trust_type,
    ...
    entity_file_upload(){
      ...
      this.service.update_type_entity(this.user_id,this.name_entity,this.title_entity,this.entity_type,
    ...
    joint_(){
      ...
      this.service.update_joint_type(this.user_id,this.joint_owner,this.joint_first_name,this.joint_last_name,this.joint_inv_address,this.joint_phone_no,this.country_code,this.autocomplete.input,
        this.state,this.postalCode).subscribe(res=>{
          console.log(res)
        })
    }
    retirement_(){
      this.service.update_retirement_type(this.retirement_value,this.user_id).subscribe(res=>{
        console.log(res)
      })
    }
    back(){
      ...
      if(page=='new-investments'){
        this.appcomponent.downSlidePre(page)
          ...
          this.appcomponent.downSlidePre('invest-info')
      }
    }
    home(){
